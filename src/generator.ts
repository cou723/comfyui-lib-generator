...
import { scanCheckpointFilesSafe, type ScanResult } from "./scanner.ts";
...
export async function generateLibrary(options: GenerateOptions) {
  const { objectInfoPath, libraryRoot, checkpointDir } = options;
  ...
  const checkpointsPath = join(generatedDir, "checkpoints.gen.ts");

  const checkpointResult = await scanCheckpointFilesSafe(checkpointDir);
  if (!checkpointResult.success) {
    console.warn(`警告: ${checkpointResult.reason}`);
  }

  const kindSet = collectKinds(nodeInfos);
  const kinds = Array.from(kindSet).sort();
  const processedNodes = prepareNodes(
    nodeInfos,
    checkpointResult.success ? checkpointResult.files : undefined
  );
  ...
  await Promise.all([
    ...
    writeCheckpointsFile(checkpointsPath, checkpointResult),
  ]);
}
...
async function writeCheckpointsFile(
  path: string,
  result: ScanResult
): Promise<void> {
  let content: string;
  ...
  if (result.success) {
    const items = result.files.map((cp) => `  "${cp}"`).join(",\n");
    content = `// Auto-generated by comfyui-lib-generator; DO NOT EDIT.
...
function prepareNodes(
  ...
  for (const [typeName, info] of Object.entries(infos)) {
    ...
    nodes.push({
      ...
      inputs: buildInputs(info, typeName, checkpointFiles),
...
function buildInputs(
  ...
  for (const [sectionName, section] of Object.entries(sections)) {
    ...
    for (const name of orderedNames) {
      ...
      if (!spec) continue;
      const initializer = buildInputInitializer(
        spec,
...
function buildInputInitializer(
  ...
  // CheckpointLoader特別扱い
  if (checkpointFiles && isCheckpointLoaderField(typeName, fieldName)) {
    return `stringInput({${renderOptions(required, {
      enumValues: checkpointFiles,
      defaultValue,
      expectedType: "string",
    })}})`;
  }
...
function isCheckpointLoaderField(typeName: string | undefined, fieldName: string | undefined): boolean {
  if (!typeName) return false;
  if (fieldName !== "ckpt_name") return false;
  return (
    typeName === "CheckpointLoaderSimple" ||
    typeName === "CheckpointLoader" ||
    typeName.includes("CheckpointLoader")
  );
}
